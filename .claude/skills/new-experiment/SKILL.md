...
2. **Add the registry entry**:
   ...
   - condition: add it under `conditions` in the series file with `train`, `tier`, `description`, any restriction keys (`eval_languages`, `exclude_eval_languages`, `benchmarks`, `bases`, `eval_variant`) and, for a training loop other than AutoScientist, `backend` with a matching `naming.backend_slugs` entry; activate it in `active_conditions`.
   - tier: to park a benchmark or a language above the minimum publishable unit (behind a gate, a floor rule or a decision), set it in the series `tiers` block (`tiers.benchmarks`, `tiers.languages`); a cell's tier is the maximum over its condition, benchmark and languages. Promoting is the same edit in reverse; never delete a condition to park it.
   - amendment: a constant the design defers to a phase (the second benchmark, the base size, the replicate count) is changed in the series file and logged in `docs/experiments.md` section 4 with the phase numbers that decided it, before the next paid run.
...
3. **Derive the matrix**: `uv run pytest tests/test_registry.py` and `uv run python -m pipeline.plan [tier=0]`; the new cells, names and commands appear with their tiers. Paste the plan into `docs/experiments.md`. Tiers are bought in order, each after its go/no-go rule (`docs/methodology.md` section 11); tier 0 is gsm8k on en, id, jv, su and min with the native, English-anchor and Indonesian-pivot arms.
...
All finetune conditions in a series must share: base models (ids from `client.autoscientist.list_models()`), `max_iterations`, `target_win_rate`, augmentation row counts, `data_format`, `training_type`, `train_on_inputs`, the pinned `hyperparams`, the `sft` block (seeds, GPU, timeout) for the transparent backend, the translation settings, the decoding settings and the `analysis` constants (primary family, equivalence bound, target detectable difference, floor and ceiling rules). These live in the series file under `autoscientist`, `sft`, `translation`, `generation` and `analysis`. Changing any constant starts a new series file; never edit a constant in place after runs exist. The exceptions are the deferred decisions above, which are amendments.
